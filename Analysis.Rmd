...
```{r lizard census}
library(car)
density_table$adults<-density_table$fem_num+density_table$mal_num
prop_list<-lapply(density_table[,c(11:14,17)],function(x) (x/density_table$liz_num)*100)
names(prop_list)<-lapply(colnames(density_table[,c(11:14,17)]),function(x) paste0("prop",sep="_",x))
...
```{r analysis}
correlations_dens<-lapply(density_table[,c(8,11,16,17,23)],function(x) with(density_table,cor.test(density,x,method="spearman")))
...
correlations_fo_exc<-lapply(density_table[,c(8,11,16,17,23)],function(x) with(density_table,cor.test(fo_exc,x,method="spearman")))
...
lapply(density_table[,c(2,5,8)],function(x) with(density_table, cor.test(x,area,method="spearman")))
